...
function Main() {
  ...
  const submitSubscribe = () => {
    if (!subscribeInfo.email || !subscribeInfo.state || !subscribeInfo.state) {
      ...
      fetchData({
        url: "https://splitaction.azurewebsites.net/api/subscribe",
        body: { ...subscribeInfo },
        method: "POST",
      })
        .then((result) => {
          if (result.message === "success") {
            setShowModal(false);
            toast.success(
              `Subscribed successfully! Your Passcode is ${
                result?.passcode ? result?.passcode : "AllStar"
              }`,
              {
  ...
  const submitPasscode = () => {
    if (!passCode) {
      ...
      fetchData({
        url:
          "https://splitaction.azurewebsites.net/api/passcodeLogin?passcode=" +
          passCode,
        method: "GET",
      })
        .then((result) => {
