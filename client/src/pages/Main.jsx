...
function Main() {
      ...
      fetchData({
        ...
        .then((result) => {
          if (result.message === "success") {
            setShowModal(false);
            toast.success(
              `Subscribed successfully! Your Passcode is${result?.passcode}`
            );
          } else {
  ...
  const submitPasscode = () => {
    if (!passCode) {
      ...
      fetchData({
        url:
          "https://samplenodedeploy.azurewebsites.net/api/passcodeLogin?passcode=" +
          passCode,
        method: "GET",
      })
        .then((result) => {
    ...
    <div className="relative w-full h-fit">
      ...
      >
        <div className="flex justify-between">
          <h1
            className="sm:text-5xl text-3xl font-bold my-2 sm:pl-8 pl-4 py-2 cursor-pointer"
            onClick={() => {
              navigate("/");
            }}
          >
        ...
        <div className="flex sm:py-6 py-4 w-full">
          ...
          <div className="w-full">
            <p className="sm:text-3xl text-2xl mt-4 px-4 sm:px-10 text-center">
              Up your game on Sports Wagering
            </p>
            <p className="sm:text-2xl text-xl sm:mt-4 mt-2 px-4 sm:px-10 text-center">
              Free wager Concensus Data
            </p>
            <div className="sm:p-12 p-6 text-[#aaa] text-2xl sm:mt-24 mt-10">
              <div className="w-full sm:w-[550px] flex">
                <p className="text-lg sm:text-2xl text-left pr-1 whitespace-nowrap pt-1 mr-4">
                  Enter Passcode
                </p>
                <input
                  type="text"
              ...
              <button
                className="bg-[#3cdf1f] text-gray-800 font-[500] py-3 w-52 border border-gray-500
